export const siteSettings = {
  ...
  siteDescription: "AI/ML engineer and researcher Noor Aldeen Almusleh works across reinforcement learning, computer vision, statistical modelling, MLOps, and computer graphics.",
...
function stripLeadingSlash(value: string) { return value.replace(/^\/+/, ""); }
...
export function absoluteSiteUrl(path = "/", site?: URL | string) {
  ...
  const root = new URL(ensureTrailingSlash(site ? String(site) : "https://nourd.uk/"));
  const base = import.meta.env.BASE_URL || "/";
  // Respect project-page base paths without doubling a path already in SITE_URL.
  if (root.pathname === "/" && base !== "/") root.pathname = ensureTrailingSlash(base);
  const cleanPath = stripLeadingSlash(path);
  return cleanPath ? new URL(cleanPath, root).toString() : root.toString();
}
export const articleTemplateLabels = {
  "case-study": "Case Study", tutorial: "Tutorial", "research-note": "Research Note",
} as const;
